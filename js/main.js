...
async function performSearch(query) {
  ...
  try {
    ...
    addToSearchHistory(query, renderSearchHistory);
...
function addMovieToRecentlyViewed(movie) {
  const nextMovies = addRecentlyViewed(movie);
...
function renderSearchHistory() {
  ...
  if (mainContainer) {
    renderHistoryList(mainContainer, getSearchHistory(), {
      ...
      onDelete: (query) => removeSearchHistoryItem(query, renderSearchHistory),
    });
    ...
    clear.addEventListener("click", () =>
      clearSearchHistory(renderSearchHistory),
    );
